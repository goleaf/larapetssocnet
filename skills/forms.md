...
- Profile edit modal saves must call `UpdateProfileAction::handleModalUpdate()`, which validates through `UpdateProfileModalRequest` before saving; keep blur-only Livewire validation proxied to the same request rules where possible.
