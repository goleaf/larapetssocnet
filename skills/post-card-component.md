...
- Keep `Delete post` behind the reusable Livewire delete trigger. It must open a confirmation modal with a short post preview, dispatch `post-delete-requested` for optimistic card removal, and run `PostDeletionCascadeService` instead of using a browser confirm or blocking form submission.
