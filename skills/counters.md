...
- Post analytics and reaction UI reads from `posts.view_count`, `posts.reactions_count`, `posts.comments_count`, `posts.shares_count`, and the configured per-type reaction counters (`paw_count`, `love_count`, `haha_count`, `wow_count`, `sad_count`, `angry_count`). Comment reaction UI reads from `comments.reactions_count`, `comments.paw_count`, and `comments.love_count`. Keep reaction services and counter rebuild services in sync whenever reaction types change.
...
- Post reaction velocity uses `post_reaction_snapshots(post_id, captured_at, reactions_count)` with one row per post/minute. Reaction writes update the current-minute snapshot, while UI reads compare the current `posts.reactions_count` against the most recent snapshot at least five minutes old.
- Reaction notification batching reads recent rows through `reactions(reactable_type, reactable_id, created_at, user_id)` and should not hydrate full post/user records just to decide whether to notify.
- Pet reaction leaderboards should read tagged posts through the `pet_post` relationship and order by `posts.reactions_count` using the published/non-deleted post index, never by aggregating reactions during the pet About tab render.
