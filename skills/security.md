...
- Rate-limit repeated failed login attempts through Laravel `RateLimiter` using the normalized credential plus IP address. Keep persisted `users.failed_login_attempts` and `users.last_failed_login_at` as known-account audit counters, not as the source of lockout timing.
...
- Password reset confirmation must resolve `/reset-password/{token}` through the token hash before rendering, keep the email read-only, use signed reset URLs in email, invalidate database sessions and remember tokens after a successful reset, queue the password-change security alert, leave the user logged out, and redirect to login with status feedback.
