...
- Do not add dark/light theme toggles, runtime application visual style switchers, or `data-theme` controls.
- Profile theming is limited to the five accessible enum/config choices in `config/profile_themes.php`; apply them through CSS custom properties on the profile root instead of arbitrary user colors.
