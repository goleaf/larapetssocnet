...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('users') && ! Schema::hasTable('social_accounts')) {
            $this->createSocialAccountsTable();
        }

        if (Schema::hasTable('auth_audit_logs')) {
            $this->repairAuthAuditLogsTable();
        }

        if (Schema::hasTable('sessions')) {
            $this->repairSessionsTable();
        }
    }
    ...
    private function createSocialAccountsTable(): void
    {
    ...
    private function repairAuthAuditLogsTable(): void
    {
        ...
        Schema::table('auth_audit_logs', function (Blueprint $table): void {
            if (
                Schema::hasColumn('auth_audit_logs', 'identifier_hash')
                && ! $this->hasIndexByName('auth_audit_logs', self::AUTH_AUDIT_FAILURE_LOOKUP_INDEX)
            ) {
                $table->index(['event_type', 'ip_address', 'identifier_hash', 'created_at'], self::AUTH_AUDIT_FAILURE_LOOKUP_INDEX);
            }
        });
    }

    private function repairSessionsTable(): void
    {
