<x-app-layout>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('marketplace.my-listings') }}" class="grid gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-4">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="{{ \App\Models\MarketplaceListing::STATUS_DRAFT }}" @selected($status === \App\Models\MarketplaceListing::STATUS_DRAFT)>Draft</option>
                        ...
                        <option value="{{ \App\Models\MarketplaceListing::STATUS_ARCHIVED }}" @selected($status === \App\Models\MarketplaceListing::STATUS_ARCHIVED)>Archived</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-4 flex justify-end gap-2">
                    <a href="{{ route('marketplace.my-listings') }}" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Reset</a>
                    ...
                    @foreach ($listings as $listing)
                        <article class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                            @if ($listing->cover_photo_url)
                                <img src="{{ $listing->cover_photo_url }}" alt="{{ $listing->title }}" class="h-44 w-full object-cover">
                            @else
                            ...
                            <div class="space-y-3 p-4">
                                <div class="flex items-start justify-between gap-2">
                                    ...
                                    <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-700">{{ ucfirst($listing->status) }}</span>
                                </div>

                                <p class="text-sm text-blue-700 font-semibold">{{ $listing->formatted_price ?: 'Price on request' }}</p>
                                <p class="text-xs text-gray-500">{{ ucfirst($listing->listing_type ?: 'listing') }} · {{ $listing->location_text ?: 'No location' }}</p>
                                <p class="text-xs text-gray-500">Views: {{ number_format((int) $listing->views_count) }}</p>

                                <div class="flex items-center gap-2">
                                    <a href="{{ route('marketplace.show', $listing) }}" class="inline-flex flex-1 items-center justify-center rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">View</a>
                                    <a href="{{ route('marketplace.edit', $listing) }}" class="inline-flex flex-1 items-center justify-center rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700">Edit</a>
                                </div>

                                <form method="POST" action="{{ route('marketplace.destroy', $listing) }}" onsubmit="return confirm('Delete this listing?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700">Delete</button>
                                </form>
                            </div>
                        </article>
                    @endforeach
