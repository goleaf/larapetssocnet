<x-app-layout>
 ...
 <div>
 <h2 class="text-xl font-semibold text-gray-400 leading-tight">My Listings</h2>
 <p class="mt-1 text-sm text-gray-400">Manage your marketplace inventory and listing status.</p>
 </div>

 <div class="flex items-center gap-2">
 <a href="{{ route('messages.index') }}"
 class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-400 hover:bg-gray-50">Messages</a>
 ...
 <label for="q"
 class="block text-xs font-semibold uppercase tracking-wide text-gray-400">Search</label>
 ...
 <div>
 <label for="status"
 class="block text-xs font-semibold uppercase tracking-wide text-gray-400">Status</label>
 ...
 <div>
 <label for="sort"
 class="block text-xs font-semibold uppercase tracking-wide text-gray-400">Sort</label>
 ...
 class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-400 hover:bg-gray-50">Reset</a>
 <button type="submit"
 ...
 @if ($listings->isEmpty())
 <div class="rounded-xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-400">
 ...
 <h3 class="font-semibold text-gray-400">{{ $listing->title }}</h3>
 <span
 class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-400">{{ ucfirst($listing->status) }}</span>
 </div>

 <p class="text-sm text-blue-700 font-semibold">
 {{ $listing->formatted_price ?:'Price on request'}}</p>
 <p class="text-xs text-gray-400">{{ ucfirst($listing->listing_type ?:'listing') }} ·
 {{ $listing->location_text ?:'No location'}}</p>
 <p class="text-xs text-gray-400">Views: {{ number_format((int) $listing->views_count) }}</p>

 <div class="flex items-center gap-2">
 <a href="{{ route('marketplace.show', $listing) }}"
 class="inline-flex flex-1 items-center justify-center rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-400 hover:bg-gray-50">View</a>
