...
<div x-data="{ open: false, confirmDelete: false, copied: false }" class="relative">
 <button
 type="button"
 class="rounded-md p-1 text-gray-400 hover:bg-gray-100"
 @click="open = !open"
 ...
 @if ($isOwn)
 <a href="{{ route('posts.edit', $post) }}" class="block rounded-md px-3 py-2 text-sm text-gray-400 hover:bg-gray-50">✏️ Edit post</a>
 <button
 type="button"
 class="block w-full rounded-md px-3 py-2 text-left text-sm text-gray-400 hover:bg-gray-50"
 @click="
...
"
 ...
 <button type="button" class="block w-full rounded-md px-3 py-2 text-left text-sm text-gray-400 hover:bg-gray-50" @click="navigator.clipboard.writeText('{{ route('posts.show', $post) }}'); copied = true; setTimeout(() => copied = false, 2000);">🔗 <span x-text="copied ?'Copied!':'Copy link'"></span></button>
 ...
 <p class="text-sm text-gray-400">Delete this post? This cannot be undone.</p>
 <div class="mt-3 flex justify-end gap-2">
 <button type="button" class="rounded-md px-3 py-1 text-sm text-gray-400 hover:bg-gray-100" @click="confirmDelete = false">Cancel</button>
 <button
 type="button"
