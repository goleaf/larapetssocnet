...
<article class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm" aria-label="Post by {{ $post->author->name }}">
 ...
 <div>
 <a href="{{ route('profile.show', $post->author) }}" class="text-sm font-semibold text-gray-400 hover:underline">{{ $post->author->name }}</a>
 ...
 @endphp
 @if (strlen($plain) > 300)
 <div class="text-sm leading-relaxed text-gray-400" x-show="!expanded">{{ Str::limit($plain, 300) }}</div>
 <div class="prose prose-sm max-w-none text-sm text-gray-400" x-show="expanded">{!! $renderedBody !!}</div>
 ...
 @else
 <div class="prose prose-sm max-w-none text-sm text-gray-400">{!! $renderedBody !!}</div>
 @endif
 </div>
 @endif
 ...
 @if ($post->location)
 <div class="px-4 py-1 text-xs text-gray-400">📍 {{ $post->location }}</div>
 @endif

 <div class="flex items-center gap-4 border-b border-gray-100 px-4 py-2 text-xs text-gray-400">
 ...
 <a href="{{ route('posts.show', $post) }}#comments" class="rounded-md px-2 py-1 text-gray-400 hover:bg-gray-100">💬 {{ $post->comments_count ?? 0 }} {{ $post->comments_count === 1 ?'Comment':'Comments'}}</a>

 <button
 type="button"
 class="rounded-md px-2 py-1 text-gray-400 hover:bg-gray-100"
