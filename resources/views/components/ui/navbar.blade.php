...
<nav class="sticky top-0 z-40 w-full border-b border-whisker/30 bg-warm-white/95 shadow-sm backdrop-blur-sm" x-data="{ mobileMenuOpen: false }" @keydown.escape.window="mobileMenuOpen = false">
        ...
        <div class="flex h-full items-center justify-between">
            ...
            <div class="hidden items-center space-x-6 md:flex">
                    ...
                    <x-ui.dropdown align="right" width="48">
                        <x-slot name="trigger">
                            ...
                            >
                                ...
                                <x-ui.avatar size="sm" :name="auth()->user()->name ?? 'User'" :src="auth()->user()?->avatar_url" />
                                <span class="hidden text-sm font-medium text-bark lg:block">{{ auth()->user()->name ?? 'User' }}</span>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4 text-fur" aria-hidden="true">
                        ...
                        <x-slot name="content">
                            ...
                            <x-ui.dropdown-item :href="Route::has('settings.profile.edit') ? route('settings.profile.edit') : '/settings'">Settings</x-ui.dropdown-item>
                            <div class="my-1 border-t border-whisker/30"></div>
                            <form method="POST" action="{{ Route::has('logout') ? route('logout') : '/logout' }}">
                                @csrf
                                <x-ui.dropdown-item variant="danger" onclick="event.preventDefault(); this.closest('form').submit();">
                                    Log Out
                                </x-ui.dropdown-item>
                            </form>
                        </x-slot>
                ...
                @else
                    <div class="flex items-center gap-3">
                        <x-ui.button :href="Route::has('login') ? route('login') : '/login'" variant="ghost" size="sm">Log In</x-ui.button>
                        <x-ui.button :href="Route::has('register') ? route('register') : '/register'" variant="primary" size="sm">Sign Up</x-ui.button>
                    </div>
                @endauth
            </div>
            ...
            <div class="flex items-center md:hidden">
                ...
                >
                    ...
                    <svg x-show="mobileMenuOpen" x-cloak class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
    ...
    >
        ...
        @auth
            <div class="border-t border-whisker/30 pb-3 pt-4">
                ...
                <div class="mt-3 space-y-1">
                    <a href="{{ Route::has('profile.show') ? route('profile.show', auth()->user()) : '/profile' }}" class="block px-4 py-2 text-base font-medium text-fur hover:bg-cream hover:text-bark">My Profile</a>
                    <a href="{{ Route::has('settings.profile.edit') ? route('settings.profile.edit') : '/settings' }}" class="block px-4 py-2 text-base font-medium text-fur hover:bg-cream hover:text-bark">Settings</a>

                    <form method="POST" action="{{ Route::has('logout') ? route('logout') : '/logout' }}">
                        @csrf
                        <button type="submit" class="block w-full px-4 py-2 text-left text-base font-medium text-rose hover:bg-rose-light">
                            Log Out
                        </button>
                    </form>
                </div>
        ...
        @else
            <div class="space-y-3 border-t border-whisker/30 p-4">
                <x-ui.button :href="Route::has('login') ? route('login') : '/login'" variant="ghost" full>Log In</x-ui.button>
                <x-ui.button :href="Route::has('register') ? route('register') : '/register'" variant="primary" full>Sign Up</x-ui.button>
            </div>
        @endauth
    </div>
</nav>
