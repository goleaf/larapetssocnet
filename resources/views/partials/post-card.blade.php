...
<article
    class="shell-card overflow-hidden p-4 sm:p-5"
    x-data="{
...
>
                ...
                <div class="min-w-0">
                    ...
                    <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs shell-text-muted">
                        ...
                        @if ($post->pet && $petUrl)
                            ...
                            <a href="{{ $petUrl }}" class="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-semibold hover:bg-emerald-50" style="border-color: var(--ui-border); color: var(--ui-primary);">
                                <span aria-hidden="true">🐾</span>
        ...
        <div class="flex shrink-0 items-center gap-2">
            ...
            @if ($isOwner)
                <form action="{{ route('posts.destroy', $post) }}" method="POST" onsubmit="return confirm('Delete this post?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="rounded-lg border border-red-200 px-2.5 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-50">
                        Delete
                    </button>
                </form>
            @endif
    ...
    <div class="mt-4 border-t pt-3" style="border-color: var(--ui-border);">
        ...
        <div x-show="commentsOpen" x-transition.opacity class="mt-3 space-y-3" style="display: none;">
                        ...
                        <div class="flex items-start justify-between gap-3">
                            ...
                            @if (auth()->id() === $comment->user_id)
                                <form action="{{ route('comments.destroy', $comment) }}" method="POST" onsubmit="return confirm('Delete this comment?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" data-testid="comment-delete" class="text-xs font-semibold text-red-600 hover:text-red-700">
                                        Delete
                                    </button>
                                </form>
                            @endif
            ...
            @else
                <a href="{{ route('login') }}" class="inline-flex text-xs font-semibold hover:underline" style="color: var(--ui-primary);">
                    Log in to comment
                </a>
            @endauth
        </div>
