                ...
                @if (count($mentionSuggestions) > 0)
                    <div class="absolute left-0 top-full z-30 mt-2 w-72 overflow-hidden rounded-[var(--radius-soft)] border border-fur/15 bg-warm-white shadow-lg">
                        @foreach ($mentionSuggestions as $suggestion)
                            <button type="button" wire:key="reply-mention-{{ $parentCommentId }}-{{ $suggestion['id'] }}" wire:click="selectMention({{ $suggestion['id'] }}, @js($suggestion['username']))" class="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-cream">
