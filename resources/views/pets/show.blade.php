    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                @if($isOwner)
                    <a href="{{ route('pets.edit', $petSlug) }}" class="text-sm text-indigo-600 hover:text-indigo-800">Edit profile</a>
                    <a href="{{ route('pets.health.index', $petSlug) }}" class="text-sm text-indigo-600 hover:text-indigo-800">Health logs</a>
                @endif
            </div>
        </div>
    </x-slot>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @if(!empty($pet->is_public))
                                <span class="inline-flex rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700">Public</span>
                            @endif
                            @if(!$isOwner && !empty($pet->is_public))
                                <span class="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">Visible profile</span>
                            @endif
                        </div>
                    </div>
                        ...
                        @if(! $isOwner)
                            <div class="text-right space-y-2">
                                ...
                                @if($followerCount !== null)
                                    <p class="text-xs text-gray-500"><span id="followers-count">{{ $followerCount }}</span> followers</p>
                                @endif
                            </div>
                        @endif
                    @endauth
                ...
                <div class="border-b border-gray-200 px-6 py-4">
                    <nav class="flex flex-wrap gap-2">
                        @foreach (['posts' => 'Posts', 'gallery' => 'Gallery', 'about' => 'About'] as $tab => $label)
                            <a
                                href="{{ route('pets.show', ['slug' => $petSlug, 'tab' => $tab]) }}"
                        ...
                        @if($isOwner)
                            <a
                                href="{{ route('pets.show', ['slug' => $petSlug, 'tab' => 'health']) }}"
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        @if($posts->isNotEmpty())
                            <div class="space-y-4">
                                @foreach($posts as $post)
                                    @php
                                        $title = data_get($post, 'title', 'Post #'.$post->getKey());
                                        $preview = data_get($post, 'body') ?: data_get($post, 'content', '');
                                    @endphp
                                    <article class="rounded-lg border border-gray-200 p-4">
                                        <h3 class="font-semibold text-gray-900">{{ $title }}</h3>
                                        <p class="mt-2 text-sm text-gray-600">{{ \Illuminate\Support\Str::limit(strip_tags((string) $preview), 180) }}</p>
                                    </article>
                                @endforeach
                            </div>
                        @else
                    ...
                    @elseif($activeTab === 'health' && $isOwner)
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('pets.health.index', $petSlug) }}" class="text-sm text-indigo-600 hover:text-indigo-800">Open health log</a>
                        </div>
