        ...
        <div class="max-w-6xl mx-auto space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <div class="flex flex-wrap items-start justify-between gap-6">
                    <div class="space-y-2">
                        <div class="text-sm text-gray-400">{{ $pet->species ?? 'Unknown species'}}
                            @if(!empty($pet->breed)) • {{ $pet->breed }} @endif</div>
                        @if($ageLabel)
                            <div class="text-sm text-gray-400">Age: {{ $ageLabel }}</div>
                        @endif
                        <div class="text-sm text-gray-400">{{ $pet->bio ?: 'No bio yet.'}}</div>
                        <div class="flex flex-wrap gap-2">
                            ...
                            @if(!$isOwner && !empty($pet->is_public))
                                <span
                                    class="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">Visible
                                    profile</span>
                            @endif
                        </div>
                    </div>
                        ...
                        @if(!$isOwner)
                            <div class="text-right space-y-2">
                                ...
                                @if($followerCount !== null)
                                    <p class="text-xs text-gray-400"><span id="followers-count">{{ $followerCount }}</span>
                                        followers</p>
                                @endif
                            </div>
                        @endif
                    ...
                    <nav class="flex flex-wrap gap-2">
                        @foreach (['posts' => 'Posts', 'gallery' => 'Gallery', 'about' => 'About'] as $tab => $label)
                            <a href="{{ route('pets.show', ['slug' => $petSlug, 'tab' => $tab]) }}"
                                class="rounded-md px-3 py-2 text-sm {{ $activeTab === $tab ? 'bg-indigo-100 text-indigo-700' : 'text-gray-400 hover:bg-gray-100'}}">
                                {{ $label }}
                            </a>
                        @endforeach
                        ...
                        @if($isOwner)
                            <a href="{{ route('pets.show', ['slug' => $petSlug, 'tab' => 'health']) }}"
                                class="rounded-md px-3 py-2 text-sm {{ $activeTab === 'health' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-400 hover:bg-gray-100'}}">
                                Health
                            </a>
                        @endif
                    </nav>
                            ...
                            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                @foreach($gallery as $item)
                                    ...
                                    <div class="rounded-lg border border-gray-200 overflow-hidden">
                                        ...
                                        @endif
                                        <div class="p-3 text-sm text-gray-400">{{ $label }}</div>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="text-sm text-gray-400">No gallery items yet.</p>
                        @endif
                    @elseif($activeTab === 'health' && $isOwner)
                        <div class="flex items-center justify-between">
                            <h3 class="text-base font-semibold text-gray-400">Recent health updates</h3>
                            <a href="{{ route('pets.health.index', $petSlug) }}"
                                class="text-sm text-indigo-600 hover:text-indigo-800">Open health log</a>
                        </div>

                        <div class="mt-4 rounded-lg border border-gray-200 bg-white p-4">
                            <h4 class="text-sm font-semibold text-gray-400">Weight history</h4>
                            <p class="mt-1 text-xs text-gray-400">Last 30 weight entries.</p>
                            ...
                            @else
                                <p class="mt-3 text-sm text-gray-400">Add at least one weight entry to see a chart.</p>
                            @endif
                        </div>
                        ...
                        @else
                            <p class="mt-2 text-sm text-gray-400">No health entries yet.</p>
                        @endif
                    @else
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <h3 class="text-sm font-semibold text-gray-400">Basic info</h3>
                                <dl class="mt-2 space-y-2 text-sm text-gray-400">
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Species:</dt>
                                        {{ $pet->species ?? 'N/A'}}
                                    </div>
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Breed:</dt> {{ $pet->breed ?? 'N/A'}}
                                    </div>
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Sex:</dt>
                                        {{ $pet->sex ?? $pet->gender ?? 'N/A'}}
                                    </div>
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Age:</dt> {{ $ageLabel ?? 'N/A'}}
                                    </div>
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Birthdate:</dt>
                                        {{ $birthdateLabel ?? 'N/A'}}
                                    </div>
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Weight:</dt>
                                        {{ $pet->weight_kg ? $pet->weight_kg . 'kg' : ($pet->weight ? $pet->weight . 'kg' : 'N/A') }}
                                    </div>
                                    <div>
                                        <dt class="inline font-medium text-gray-400">Color:</dt> {{ $pet->color ?? 'N/A'}}
                                    </div>
                                </dl>
                            </div>

                            <div>
                                <h3 class="text-sm font-semibold text-gray-400">Personality</h3>
                                @if(!empty($personalityTags))
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        @foreach($personalityTags as $tag)
                                            <span
                                                class="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{{ trim((string) $tag) }}</span>
                                        @endforeach
                                    </div>
                                @else
                                    <p class="mt-2 text-sm text-gray-400">No personality tags yet.</p>
                                @endif
                            </div>
                        </div>
                    @endif
