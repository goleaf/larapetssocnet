@section('title', __('messaging.messages.show.title', ['name' => $peer->name]))

<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header
            :title="__('messaging.messages.show.heading')"
            :subtitle="__('messaging.messages.show.subtitle', ['name' => $peer->name])"
            :breadcrumbs="[
                ['label' => __('messaging.messages.index.heading'), 'href' => route('messages.index')],
                ['label' => $peer->name],
            ]"
            icon="💬"
        />
                    ...
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="truncate text-sm text-fur">
                                {{ $peer->username ? '@'.$peer->username : __('messaging.messages.index.default_peer_label') }}
                            </p>
                        </div>
                    </div>

                    <x-ui.button variant="outline" size="sm" :href="route('messages.index')" icon="↩️">
                        {{ __('messaging.messages.actions.back_to_inbox') }}
                    </x-ui.button>
                </div>
            </header>
            ...
            <section class="bg-gradient-to-b from-cream/60 via-warm-white to-cream/70">
	                <div class="h-[58vh] min-h-[22rem] overflow-y-auto px-4 py-4 sm:px-5">
                        ...
                        @empty
                            <x-ui.empty-state
                                icon="💌"
                                :title="__('messaging.messages.show.empty_title')"
                                :description="__('messaging.messages.show.empty_description')"
                            />
                        @endforelse
                    </div>
                </div>
            ...
            <footer class="border-t border-whisker/30 bg-warm-white px-4 py-3 sm:px-5">
                <form method="POST" action="{{ route('messages.store', ['peer' => $peer]) }}" class="space-y-3">
                    ...
                    <x-ui.textarea
                        ...
                        maxlength="5000"
                        required
                        :placeholder="__('messaging.messages.show.input_placeholder')"
                        :value="old('body')"
                        :error="$errors->first('body')"
                    />

                    <div class="flex items-center justify-between gap-3">
                        <p class="text-xs text-fur">{{ __('messaging.messages.show.helper') }}</p>
                        <x-ui.button type="submit" variant="primary" size="sm" icon="➤">
                            {{ __('messaging.messages.actions.send') }}
                        </x-ui.button>
                    </div>
                </form>
            </footer>
