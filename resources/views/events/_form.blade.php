...
<div class="space-y-5">
 ...
 class="mt-1 block w-full rounded-md border border-[var(--ui-border)] bg-white px-3 py-2 text-sm"
	 ...
	 <option value="{{ $group->id }}" @selected((string) old('group_id', $selectedGroupId ?? data_get($event, 'group_id')) === (string) $group->id)>
	 {{ $group->name }}
	 </option>
	 @endforeach
 </select>
 ...
 <x-input-label for="starts_at" :value="'Starts At'"/>
	 <x-text-input id="starts_at" name="starts_at" type="datetime-local" class="mt-1 block w-full" :value="old('starts_at', $startAt ? $startAt->format('Y-m-d\\TH:i') : now()->addDay()->format('Y-m-d\\TH:i'))" required />
 <x-input-error :messages="$errors->get('starts_at')" class="mt-2"/>
 </div>

 <div>
 <x-input-label for="ends_at" :value="'Ends At (optional)'"/>
	 <x-text-input id="ends_at" name="ends_at" type="datetime-local" class="mt-1 block w-full" :value="old('ends_at', $endAt ? $endAt->format('Y-m-d\\TH:i') : '')"/>
