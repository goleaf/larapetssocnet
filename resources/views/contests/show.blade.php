...
'ended'=>'bg-gray-100 text-gray-600',
 ...
 <p class="text-gray-700 mb-4 leading-relaxed">{{ $contest->description }}</p>
 @endif

 <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500 border-t border-gray-100 pt-4">
 ...
 <span class="font-semibold text-sm text-gray-900">{{ $entry->user->name ??'User'}}</span>
 @if ($entry->is_winner) <span
 class="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full font-medium">🥇
 Winner</span> @endif
 </div>
 @if ($entry->caption)
 <p class="text-sm text-gray-600 mb-3">{{ $entry->caption }}</p>
 @endif
