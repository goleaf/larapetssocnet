...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=outfit:500,600,700,800|nunito-sans:400,500,600,700&display=swap"
        rel="stylesheet"  />
...
<body class="min-h-screen bg-cream font-body text-bark antialiased" x-data="appShell()">
    <div class="relative min-h-screen">
        ...
        <div class="pointer-events-none absolute inset-x-0 top-0 z-0 h-[28rem] overflow-hidden">
            <div class="absolute -left-16 top-12 h-72 w-72 rounded-full opacity-40 blur-3xl animate-float bg-paw-light">
            </div>
            <div class="absolute -right-16 top-0 h-80 w-80 rounded-full opacity-35 blur-3xl animate-float bg-amber-light"
                    ...
                    <div class="sticky top-24 space-y-4">
                        <x-ui.card>
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="truncate text-base font-semibold text-bark">{{ $user?->name ?? 'Guest User'}}
                                    </p>
                                    <p class="truncate text-xs text-fur">{{ $user?->username ?'@'.$user->username :
                                        ($user?->email ??'community@larapets.test') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 grid grid-cols-3 gap-2">
                                @foreach ($communityStats as $stat)
                                    <div class="rounded-xl border border-whisker/30 bg-warm-white px-2 py-2 text-center">
                                        ...
                                        <p class="text-[0.62rem] font-semibold uppercase tracking-[0.08em] text-fur">
                                            {{ $stat['label'] }}</p>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <x-ui.card>
                            <h4 class="px-1 text-xs font-bold font-display uppercase tracking-wider text-fur mb-2">Navigate
                            </h4>
                            @php
                        ...
                        <x-ui.card>
                            <x-slot name="header">
                                <x-ui.card-header title="Your Groups" subtitle="Communities you are active in">
                                    <x-slot name="action">
                                        <x-ui.button href="{{ route('groups.index', ['privacy' => 'joined']) }}"
                            ...
                            <div class="space-y-1 -mx-2">
                                @forelse ($yourGroups as $group)
                                    ...
                                    <x-ui.user-row :name="$group->name" :subtitle="\Illuminate\Support\Str::headline((string) ($group->privacy ?? 'public'))" :href="route('groups.show', $groupRouteKey)"
                                        class="px-2">
                                        <x-slot name="action">
                        ...
                        <x-ui.card>
                            ...
                            <div class="space-y-2">
                                @forelse ($suggestedUsers as $suggestedUser)
                                    <x-ui.user-row :name="$suggestedUser->name" :subtitle="$suggestedUser->username ? '@' . $suggestedUser->username : 'Pet lover'" :href="route('profile.show', $suggestedUser)">
                                        <x-slot name="avatar">
                                            <x-ui.avatar :src="$suggestedUser->avatar_url" :name="$suggestedUser->name"
                                                size="sm"  />
                                        </x-slot>
                                    </x-ui.user-row>
                                @empty
                        ...
                        <x-ui.card>
                            <div class="mb-3 flex items-center justify-between">
                                <h4 class="text-xs font-bold font-display uppercase tracking-wider text-fur">Who To Follow
                                </h4>
                                <a href="{{ Route::has('search.index') ? route('search.index', ['type' => 'users']) : '#'}}"
                            ...
                            <div class="space-y-1 -mx-2">
                                @forelse ($suggestedUsers as $suggested)
                                    <x-ui.user-row :name="$suggested->name" :subtitle="$suggested->username ? '@' . $suggested->username : 'Pet lover'" :avatar="$suggested->avatar_url"
                                        :href="route('profile.show', ['user' => $suggested])" class="px-2">
                                        <x-slot name="action">
                                            <span
                                                class="text-xs text-fur">{{ number_format((int) $suggested->followers_count) }}</span>
                                        </x-slot>
                                    </x-ui.user-row>
                                @empty
                        ...
                        <x-ui.card>
                            <div class="mb-3 flex items-center justify-between">
                                <h4 class="text-xs font-bold font-display uppercase tracking-wider text-fur">Upcoming Events
                                </h4>
                        ...
                        <x-ui.card>
                            <div class="mb-3 flex items-center justify-between">
                                <h4 class="text-xs font-bold font-display uppercase tracking-wider text-fur">Active Contests
                                </h4>
                            ...
                            <div class="space-y-2 mt-3">
                                @forelse ($activeContests as $contest)
                                    <a href="{{ Route::has('contests.index') ? route('contests.index') : '#'}}"
                                        class="block rounded-xl border border-whisker/30 bg-warm-white px-3 py-2 hover:bg-cream transition-colors shadow-sm">
                        ...
                        <x-ui.card>
                            <div class="mb-2 flex items-center justify-between">
                                <h4 class="text-xs font-bold font-display uppercase tracking-wider text-fur">Trending Tags
                                </h4>
                            ...
                            <div class="space-y-2 mt-3">
                                @forelse ($trendingHashtags as $hashtag)
                                    <a href="{{ route('hashtags.show', $hashtag) }}"
        ...
        <nav class="fixed inset-x-3 bottom-3 z-40 lg:hidden">
            <div
                class="bg-warm-white rounded-2xl shadow-card-hover border border-whisker/30 flex items-center justify-between px-2 py-1.5">
