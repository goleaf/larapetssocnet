...
@section('content')
 ...
 <h1 class="text-2xl font-bold text-gray-400">🚩 Reports</h1>
 <a href="{{ route('admin.dashboard') }}" class="text-sm text-gray-400 hover:text-gray-400">← Dashboard</a>
 </div>

 <div class="flex gap-2 mb-6">
 @foreach (['pending','reviewed','dismissed','actioned'] as $s)
 <a href="{{ route('admin.reports.index', ['status'=> $s]) }}"
 class="rounded-lg px-3 py-1.5 text-xs font-medium {{ $status === $s ?'bg-emerald-100 text-emerald-700':'bg-gray-100 text-gray-400 hover:bg-gray-200'}}">
 {{ ucfirst($s) }}
 </a>
 @endforeach
 </div>
 ...
 <div>
 <p class="text-sm font-medium text-gray-400">{{ $report->reason ??'No reason'}}</p>
 ...
 <button class="rounded bg-gray-100 px-2 py-1 text-xs text-gray-400">Dismiss</button>
 </form>
 <form x-data
